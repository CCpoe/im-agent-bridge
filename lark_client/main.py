...
def handle_message_receive(data: P2ImMessageReceiveV1) -> None:
    ...
    try:
        ...
        asyncio.create_task(handler.handle_message(
            user_id,
            chat_id,
            text,
            chat_type=chat_type,
...
def handle_card_action(event: P2CardActionTrigger) -> P2CardActionTriggerResponse:
    ...
    try:
        ...
        if form_value is not None:
            desktop_field = next(
                (key for key in form_value if key.startswith("desktop_command__")),
                None,
            )
            if desktop_field is not None:
                thread_id = desktop_field.removeprefix("desktop_command__")
                command_text = (form_value.get(desktop_field) or "").strip()
        ...
        if action_type == "desktop_list":
            asyncio.create_task(handler._cmd_desktop_list(user_id, chat_id, message_id=message_id))
            return None

        if action_type == "desktop_list_page":
            try:
                page = int(action_value.get("page", 0))
            except (TypeError, ValueError):
                page = 0
            print(f"[Lark] desktop_list_page: page={page}")
            asyncio.create_task(handler._cmd_desktop_list(
        ...
        if action_type == "desktop_detach":
            asyncio.create_task(handler._cmd_desktop_detach(
                user_id, chat_id, message_id=message_id,
                expected_thread_id=action_value.get("thread_id") or None,
            ))
            return None

        if action_type == "desktop_interrupt":
            asyncio.create_task(handler._cmd_desktop_stop(
                user_id, chat_id,
                expected_thread_id=action_value.get("thread_id") or None,
        ...
        if action_type == "desktop_approval":
            asyncio.create_task(handler.handle_desktop_approval(
                user_id,
                chat_id,
                action_value.get("kind", ""),
        ...
        if action_type == "desktop_input":
            asyncio.create_task(handler.handle_desktop_input(
                user_id,
                chat_id,
                action_value.get("kind", ""),
                action_value.get("request_id", ""),
                action_value.get("answer", ""),
                question_id=action_value.get("question_id") or None,
                expected_thread_id=action_value.get("thread_id") or None,
            ))
            return None

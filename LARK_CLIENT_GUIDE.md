...
本指南介绍如何使用 im-agent-bridge 的飞书客户端管理功能。

## Codex Desktop 双向控制

ChatGPT/Codex Desktop 运行时会创建 `~/.codex/ipc/ipc.sock`。飞书客户端以
follower 身份连接这个本机 socket，由 Desktop 中持有线程的 owner 代为执行：

- 空闲线程：开始新 turn；
- 运行中线程：追加 steer 消息；
- 实时状态：更新同一张飞书卡片；
- 命令/文件审批与用户选择：通过卡片按钮回传；
- 停止：中断当前 turn，不关闭 Desktop 任务。

在飞书发送 `/desktop` 选择任务。列表每页显示 5 个任务，并展示 Desktop 项目名、
完整工作目录和 Session ID，可通过卡片底部按钮翻页。连接后直接发送普通消息即可，
也可使用完整 `codex://threads/<thread-id>` 链接连接指定任务。

若出现“无法连接 Codex Desktop”，请确认 Desktop 已运行，并检查：

```bash
ls -l ~/.codex/ipc/ipc.sock
```

该 socket 必须属于当前用户。此模式不会监听公网端口。
...
2. 提交 Issue：[GitHub Issues](https://github.com/CCpoe/im-agent-bridge/issues)

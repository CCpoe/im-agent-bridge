...
当前分支同时支持 Codex Desktop owner/follower 模式：飞书客户端通过 Desktop 的
本机 IPC 连接现有 GUI 线程，由该线程的 owner 执行 start/steer/interrupt/approval，
不会另起 CLI 抢占 writer lock。
...
DesktopBridgeManager
      │
DesktopIPCClient ── ~/.codex/ipc/ipc.sock
      │
ChatGPT Desktop owner ── stdio App Server ── 目标 thread
```
...
- `desktop_ipc.py` — Desktop 私有 IPC framing、owner discovery、follow、start/steer、审批
- `desktop_bridge.py` — chat→Desktop thread 绑定、rollout 基线、IPC 增量和卡片更新
...
- 所有卡片上的 `desktop_attach` 都优先复用回调 `context.open_message_id` 对应的
  ...
  当前聊天的活动卡。
- 完成提醒只保留一个连接入口：整个 `NEXT / 重新连接` 高亮区域触发
  `desktop_attach + thread_id`，不再在 Session 详情下方重复显示连接按钮。
  缺少有效 Session ID 时不提供连接回调；任务列表的高亮统计区域仍保持非交互。
- 完成提醒只由主线程自身的 `task_complete` 触发。扫描前与持久化重试发送前，
  均须核对 rollout 首条 `session_meta` 的自身 `id` 与来源线程 ID 一致，并排除
  父线程字段、`thread_source` 以及对象/JSON 字符串形式的 `source.subagent`。
  子线程复用的 `session_id` 不得冒充自身 ID；无法确认身份时暂缓，明确为子任务的
  旧待发通知应丢弃，但不能重放已经发送或订阅前的历史完成事件。
  待发记录须固定原始来源路径及已验证的根身份；升级时先保留旧 cursor 的来源证据，
  再迁移 cursor，禁止用新根路径为旧子任务事件背书。只有已验明根身份的记录可跨
  归档路径恢复发送。
- 会话详情按父轮展示“子 Agent 状态”，只投影 `subAgentActivity` 的线程 ID、
  任务路径和活动类型，最多 32 个。`interacted` 不等于重新运行，应标记待同步，
  再结合子 rollout 自身 `ordinal > subagent_history_start_ordinal` 的生命周期确认。
  仅最新父轮补充实时子状态，历史轮不跟随子任务后续轮次变化；子任务完成、失败或
  中断不得修改父任务状态、审批或通知。绑定期间每 2 秒检查运行中/待同步的子任务，
  复用原卡更新；读取有界、按文件状态缓存，未知或超限时显示待同步。
